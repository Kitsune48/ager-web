...
export async function GET(req: Request) {
  ...
  if (!res.ok) {
    ...
    // Pass-through body when possible (ProblemDetails or text)
    try {
      const json = errText ? JSON.parse(errText) : null;
      return NextResponse.json(json ?? { message: errText || "Search failed" }, { status: res.status });
    } catch {
      return NextResponse.json({ message: errText || "Search failed" }, { status: res.status });
    }
  }
